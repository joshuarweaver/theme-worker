...
// Combined TOC Scroller and Progress Bar
(function() {
  ...
  function init() {
    ...
    // Efficient scroll handling with debounce
    let scrollTimeout;
    window.addEventListener('scroll', function() {
      if (scrollTimeout) clearTimeout(scrollTimeout);
      scrollTimeout = setTimeout(function() {
        ${isPostPage ? 'updateProgress();' : ''}
        if (tocLinks && tocLinks.length > 0) updateActiveLink();
      }, 100);
    });
  ...
  // Initialize when DOM is ready or after load
